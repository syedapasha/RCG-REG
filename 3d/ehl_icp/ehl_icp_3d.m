function [R, t, errors, n_iter, Xreg] = ehl_icp_3d(X, Y, varargin)
% ... [header unchanged]

opts = inputParser;
addParameter(opts, 'max_iter', 30);
addParameter(opts, 'min_iter', 3);
addParameter(opts, 'eta', 0.5);
addParameter(opts, 'mu', 0.1);
...
J = zeros(3); % Hamiltonian generator (so(3))
...
for k = 1:max_iter
    ...
    % Step 5: Hamiltonian update using exponential map with momentum
    J = (1 - eta*mu)*J + eta * (R' * G); % <-- FIXED SIGN HERE!
